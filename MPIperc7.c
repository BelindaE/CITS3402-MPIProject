...
STACK initialise_stack(STACK stack){
	//#pragma omp critical
	{
		stack.x = 0;
		stack.y = 0;
...
void push(STACK cluster[], int top, int i, int j){
    cluster[top].x = j;
    cluster[top].y = i;
}

STACK pop(STACK cluster[], int top){
    popped = true;
    return cluster[top--];
}

STACK peek(STACK cluster[], int top){
    return cluster[top];
}
...
void displayNode(STACK cluster[], int top){
    printf("(%i, %i)\t", peek(cluster, top).x, peek(cluster,top).y);               //prints out map co-ordinate
...
NODE getNextNode(STACK cluster[], int top, int taskid, int offset) {
    popped = false;
    int j = peek(cluster, top).x ;
    int i = peek(cluster, top).y ;
    ...
    if ((i+1 < (offset/L+P)) && map[i+1][j].flag == 1 && map[i+1][j].up == 0) {
        ...
        //printf("CurrentID = %d\n",currentID);
        clusterMap[peek(cluster, top).y][peek(cluster,top).x] = clusterID;
        deadEnd(peek(cluster,top).y, peek(cluster,top).x);                    // changes path status to 2
        pop(cluster,top);
		popped = true;
...
int depthFirstSearch(int i, int j, int taskid, int offset){
    ...
    int tempY = 0;
    int tempC = 0;
	printf("getting here dfs taskid %d\n",taskid);
    STACK cluster[MAPSIZE];
	for (m=0; m < MAPSIZE; m++){
		initialise_stack(cluster[m]);
	}
    ...
    while(!isStackEmpty(top)){
        ...
        push(cluster, top, cluster[top].y, cluster[top].x);                   // push node into the stack
        tempX = cluster[top].x;
        tempY = cluster[top].y;
        tempC = clusterID;
		printf("getting here dfs taskid %d clusterID %d count %d\n",taskid, clusterID, count);
        
        temp[0][0] = getNextNode(cluster, top, taskid, offset);                  // allocate new node coords
        i = temp[0][0].y;
        j = temp[0][0].x;
        int f = temp[0][0].flag;
        //printf("f = %d, taskid = %d\n",f, taskid);
		if (popped)
	        printf("is popped popping\n"); 
        if(tempC != f)
        {
