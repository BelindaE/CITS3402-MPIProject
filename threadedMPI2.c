...
//  
//
//  Created by Belinda Edmunds on 31/10/2017.
//
//
...
#define NTHREADS 4
#define P L/NTHREADS	/* side length of single cluster map */
 ...
 */
int depthFirstSearch(int i, int j, int clusterID, int thread_num, struct Node map[P][L], struct Clusters clusterMap[NTHREADS]){
...
void matchClusters(float probability, char seedType, int perc, struct Clusters clusterMap[NTHREADS]){
    int n;
    int max = 0;
    int rows = 0;
    int cols = 0;
...
struct Clusters searchControl(int thread_num, double probability, char seedType, struct Node map[NTHREADS][P][L], struct Clusters clusterMap){
    int i, j, dfs;
    int clusterID = 2;
...
int main(int argc, char *argv[]){
    ...
    else
    {
        ...
        int numtasks;
        int taskid, dest, tag1, broadcast;
        ...
        if (taskid == MASTER) {
            ...
            searchControl(taskid, p, seedType, map, clusterMap);
        }
