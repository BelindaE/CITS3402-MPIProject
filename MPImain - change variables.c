...
#pragma omp parallel num_threads(NTHREADS)
        {
            ...
            for(i=1; i<numtasks; i++) {
                ...
                if(seedType == 'b'){
                    //MPI_Recv(&downOffset, 1, MPI_INT, source, tag5, MPI_COMM_WORLD, &status);
...
#pragma omp parallel num_threads(NTHREADS)
            {
                ...
                int thread_num = omp_get_thread_num();
                printf("threaded num = %d\n",thread_num);
                searchControl(thread_num, p, seedType, map, clusterMap, bRight, Bdown);
            }
            ...
            MPI_Send(&clusterMap[offset][0], partition , MPI_INT, dest, tag1, MPI_COMM_WORLD);
            if(seedType == 'b'){
                //MPI_Send(&downOffset, 1, MPI_INT, dest, tag5, MPI_COMM_WORLD);
