...
#define L 20            /* Linear edge dimension of map */
#define NTHREADS 5
#define P L/NTHREADS
...
void seedBond(double probability, int thread_num, struct Node map[P][L])
{
    double r1, r2;
    int seeded1, seeded2;
    int i, j;
...
struct Node getNextNode(int thread_num, struct Node map[P][L]) {
    popped = false;
    int j = peek().x ;
    int i = peek().y ;
 ...
 */
int depthFirstSearch(int i, int j, int clusterID, int thread_num, struct Node map[P][L]){
    ...
    while(!isStackEmpty()){
        ...
        temp[0][0] = getNextNode(thread_num, map);                  // allocate new node coords
        i = temp[0][0].y;
        j = temp[0][0].x;
        int f = temp[0][0].flag;
...
void matchClusters(float probability, char seedType, int perc){
    int n;
    int max = 0;
    int rows = 0;
    int cols = 0;
...
void searchControl(int thread_num, double probability, char seedType, struct Node map[NTHREADS][P][L]){
    int i, j, dfs;
	//clusterID++;
	...
	//struct Clusters clusterMap[NTHREADS]
            ...
            if (seedType == 's') {
                ...
                seedBond(probability, thread_num, map[thread_num]);
                //printBonds();
            }
            
                for(j =0; j<L; j++){
                    for(i =0; i<P; i++){
                        dfs = depthFirstSearch(i,j, clusterID, thread_num, map[thread_num]);
						if (dfs>0) clusterID++;
                    }
                }
...
int main(int argc, char *argv[]){
    ...
    else
    {
        ...
        omp_set_num_threads(NTHREADS);
	        ...
	        searchControl(thread_num, p, seedType, map);
			}
        matchClusters(p, seedType, perc);
		printf("perc = %i\n", perc);
