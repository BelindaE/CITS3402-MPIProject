...
#define L 240            /* Linear edge dimension of map */
#define MAPSIZE L*L
#define NTHREADS 1
#define NPROCESSORS 24
#define NODES 2
#define threads 24
...
void cluster_size(int idCounter[MAPSIZE],int clusterMap[][L]){
    int i,j;
    for(i = 0; i < L ; i++)
    {
        for(j = 0; j < L ; j++){
            if(clusterMap[i][j] != 0)
            {
                idCounter[clusterMap[i][j]] = idCounter[clusterMap[i][j]]  + 1;
            }
        }
    }
}
...
void matchClusters(float probability, char seedType, int perc, int clusterMap[][L], int bRight[][P], int bDown[][L], int idCounter[MAPSIZE]){
    ...
    for (n=0; n<threads; n++){
        ...
        for(i = 0; i < L; i++){
            if(n == threads-1 && clusterMap[0][i] > 0 && clusterMap[n*P + P-1][i] > 0){
...
int main(int argc, char *argv[]){
    ...
    else
    {
        ...
        if(taskid == MASTER) {
            ...
            {
                for(g = 0; g < L; g++){
					if (clusterMap[m][g] < 10) printf("---%d",clusterMap[m][g]);
					else if (clusterMap[m][g] < 100) printf("--%d",clusterMap[m][g]);
                    else printf("-%d",clusterMap[m][g]);
                }
                printf("\n");
            }
            printf("\n");*/
        ...
        if (taskid == MASTER) {
            ...
            deltaMid = ((middle.tv_sec  - start.tv_sec) * 1000000u +
                     middle.tv_usec - start.tv_usec) / 1.e6;
            printf("time after dfs=%12.10f\n",deltaMid);
            int idCounter[MAPSIZE] = {0};
            cluster_size(idCounter, clusterMap);
