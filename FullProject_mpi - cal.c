...
int **allocate_2d(int row, int cols) {
    int j;
    int *data = (int *)malloc(rows*cols*sizeof(int));
    int **array= (int **)malloc(rows*sizeof(int*));
    for (j=0; j<rows; j++) array[j] = &(data[cols*j]);
...
void matchClusters(float probability, char seedType, int perc, int **clusterMap){
    int max = 0;
    int rows = 0;
    int cols = 0;
    int both = 0;
